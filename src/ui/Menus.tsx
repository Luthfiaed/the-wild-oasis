...
function Menus({ children }: { children: any }) {
...
function Toggle({ name }: { name: string }) {
  ...
  function handleClick(e: any) {
    // todo fix this typing and find out more
...
function List({ name, children }: { name: string; children: ReactNode }) {
  const { openName, position } = useContext(MenusContext);

  if (openName !== name) return null;

  return createPortal(
    // todo fix the fixed position behavior
